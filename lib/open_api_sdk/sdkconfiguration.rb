...
module OpenApiSDK
  ...
  class SDKConfiguration
    ...
    def initialize(client, hooks, retry_config, timeout_ms, access_token, security_source, server_url, server)
      @client = client
      @hooks = hooks
      @retry_config = retry_config
      @server_url = server_url
      @timeout = (timeout_ms.to_f / 1000) unless timeout_ms.nil?
      @server = server.nil? ? SERVER_PRODUCTION : server
      raise StandardError, "Invalid server \"#{server}\"" if !SERVERS.key?(@server)
      if !security_source.nil?
        @security_source = security_source
      elsif !access_token.nil?
        @security_source = -> { Models::Components::Security.new(access_token: access_token) }
      end
      ...
      @sdk_version = "0.1.2"
      @gen_version = "2.879.6"
      @user_agent = "speakeasy-sdk/ruby 0.1.2 2.879.6 0.1.0 spaire"
    end
